...
import { MTimeline, MTimelineItem, type TimelineItem, type TimelineItemScope } from ".";
...
const Demo = defineComponent({
  setup() {
    const items = ref<TimelineItem[]>([...events]);
    const reverse = ref(false);
    return () =>
      h("div", [
        h(MTimeline, { items: items.value, pending: "进行中", reverse: reverse.value }),
        h(
          "button",
          { onClick: () => (items.value = [...items.value, { label: "申时", content: "钤印" }]) },
          "追加",
        ),
        h("button", { onClick: () => (reverse.value = !reverse.value) }, "倒序"),
      ]);
  },
});
...
describe("MTimeline", () => {
  it("renders one item per entry with label, content and type", async () => {
    const screen = await render(MTimeline, { props: { items: events } });
  ...
  it("puts content on the left in right mode and alternates in alternate mode", async () => {
    const right = await render(MTimeline, { props: { items: events, mode: "right" } });
    expect(
      itemsOf(right.container).every((el) => el.classList.contains("m-timeline-item--left")),
    ).toBe(true);
    const alt = await render(MTimeline, { props: { items: events, mode: "alternate" } });
  ...
  it("collects MTimelineItem children in written order with their slots", async () => {
    const screen = await render(MTimeline, {
      slots: {
        default: () => [
          h(
            MTimelineItem,
          ...
          h(MTimelineItem, { label: "辛丑", content: "始画山水", dot: "二" }),
          h(
            MTimelineItem,
            { key: "yin", label: "壬寅" },
            { default: () => "第一次刻印", dot: () => h("i", { class: "seal" }, "印") },
          ),
        ],
      },
    });
  ...
  it("renders the dot and item scoped slots for every entry", async () => {
    const screen = await render(MTimeline, {
      props: { items: events },
      slots: {
        dot: ({ index }: TimelineItemScope) => String(index + 1),
        item: ({ item }: TimelineItemScope) => h("em", `${item.label}·${item.content}`),
      },
    });
    const items = itemsOf(screen.container);
  ...
  it("spans the axis from the first node centre to the last real node centre", async () => {
    const screen = await render(MTimeline, { props: { items: events, pending: "进行中" } });
  ...
  it("draws one vertical brush axis across the nodes and none for a single item", async () => {
    ...
    const single = await render(MTimeline, { props: { items: events.slice(0, 1) } });
