...
import { MBackTop } from ".";

/** 一个 200px 高、内容 2000px 的滚动盒子，按钮盯着它 */
const Host = defineComponent({
  props: {
    backTopProps: { type: Object as PropType<Record<string, unknown>>, default: () => ({}) },
    onClick: { type: Function as PropType<(event: MouseEvent) => void>, required: false },
  ...
  setup(props) {
    return () =>
      h("div", [
        ...
        h(
          MBackTop,
          { target: "#box", visibilityHeight: 100, ...props.backTopProps, onClick: props.onClick },
          props.custom ? { default: () => "↑ 顶" } : undefined,
        ),
      ]);
  },
});
...
describe("MBackTop", () => {
  it("stays hidden until the target scrolls past visibilityHeight, then appears and scrolls back", async () => {
    const onClick = vi.fn();
    const screen = await render(Host, { props: { onClick } });
  ...
  it("puts right / bottom into the position variables, px for numbers and as-is for strings", async () => {
    await render(Host, { props: { backTopProps: { right: 24, bottom: "10vh" } } });
    scrollBoxTo(600);
  ...
  it("renders the default slot instead of the stamp and accepts a function target", async () => {
    const screen = await render(Host, {
      props: {
        custom: true,
        backTopProps: { target: () => document.querySelector<HTMLElement>("#box")! },
      },
    });
    scrollBoxTo(600);
    const button = screen.getByRole("button", { name: "回到顶部" });
