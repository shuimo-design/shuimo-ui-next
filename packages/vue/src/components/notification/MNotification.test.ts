...
describe("MNotification", () => {
  ...
  it("renders title, content, type class and a badge icon", async () => {
    const screen = await render(MNotification, {
      props: { type: "success", title: "保存成功", content: "已同步到云端", duration: 0 },
    });
  ...
  it("uses role=alert for errors and skips the badge without a type", async () => {
    const screen = await render(MNotification, {
      props: { type: "error", title: "出错了", duration: 0 },
    });
    await expect.element(screen.getByRole("alert")).toHaveTextContent("出错了");
    const plain = await render(MNotification, { props: { title: "无类型", duration: 0 } });
  ...
  it("emits close after clicking the close button and hides it when not closable", async () => {
    const onClose = vi.fn();
    const screen = await render(MNotification, {
      props: { title: "手动关", duration: 0, onClose },
    });
    await screen.getByRole("button", { name: "关闭" }).click();
    await vi.waitFor(() => {
      expect(onClose).toHaveBeenCalledTimes(1);
    });
    const sticky = await render(MNotification, {
      props: { title: "不可关", duration: 0, closable: false },
    });
  ...
  it("closes from the keyboard through the close button", async () => {
    const onClose = vi.fn();
    const screen = await render(MNotification, {
      props: { title: "键盘关", duration: 0, onClose },
    });
  ...
  it("pauses the countdown while hovered", async () => {
    const onClose = vi.fn();
    const screen = await render(MNotification, {
      props: { title: "悬停暂停", duration: 800, onClose },
    });
    const item = screen.getByRole("status");
