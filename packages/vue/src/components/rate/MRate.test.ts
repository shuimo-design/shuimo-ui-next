...
describe("MRate", () => {
  it("renders a radiogroup and sets the value on click", async () => {
    ...
    const screen = await render(Host, { props: { initial: 0, extra: { onChange } } });
  ...
  it("clears when the current value is clicked again", async () => {
    const screen = await render(Host, { props: { initial: 3 } });
  ...
  it("keeps the value on a repeated click when allowClear is off", async () => {
    const screen = await render(Host, { props: { initial: 3, extra: { allowClear: false } } });
    await screen.getByRole("radio").nth(2).click();
    await expect.element(screen.getByTestId("out")).toHaveTextContent("3");
  });
  ...
  it("picks half a cell from the left side when allowHalf is on", async () => {
    ...
    const screen = await render(Host, {
      props: { initial: 0, extra: { allowHalf: true, onHoverChange: onHover } },
    });
  ...
  it("adjusts with the keyboard and keeps a single tab stop", async () => {
    const onChange = vi.fn();
    const screen = await render(Host, { props: { initial: 2, extra: { onChange } } });
    const radios = screen.getByRole("radio");
  ...
  it("steps by half with the keyboard when allowHalf is on", async () => {
    const screen = await render(Host, { props: { initial: 1, extra: { allowHalf: true } } });
  ...
  it("does not respond when disabled", async () => {
    const onChange = vi.fn();
    const screen = await render(Host, {
      props: { initial: 2, extra: { disabled: true, onChange } },
    });
    const group = screen.getByRole("radiogroup");
  ...
  it("does not respond when readonly", async () => {
    const screen = await render(Host, { props: { initial: 2, extra: { readonly: true } } });
  ...
  it("shows the text for the displayed value and names each cell after it", async () => {
    ...
    const screen = await render(MRate, { props: { modelValue: 4, texts, count: 5 } });
  ...
  it("renders a custom character with the scope and a blob mask per cell", async () => {
    const screen = await render(MRate, {
      props: { modelValue: 2, count: 3, seed: 7 },
      slots: {
        character: ({ index, active }: { index: number; active: boolean }) =>
          h("i", { class: "custom", "data-active": String(active) }, String(index)),
      },
    });
