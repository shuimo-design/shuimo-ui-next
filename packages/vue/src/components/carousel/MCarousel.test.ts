...
import { MCarousel, MCarouselItem, type VueCarouselItem } from ".";
...
const Host = defineComponent({
  props: {
    ...
    /** 用数据数组而不是子组件写法 */
    asItems: { type: Boolean, default: false },
  },
  setup(props) {
    const current = ref(props.initial);
    const items: VueCarouselItem[] = NAMES.map((name) => ({
      key: name,
      render: () => h("p", `${name}的内容`),
    }));
    return () =>
        ...
        h(
          ...
          props.asItems
            ...
            : {
                default: () =>
                  NAMES.map((name) =>
                    h(MCarouselItem, { key: name }, { default: () => h("p", `${name}的内容`) }),
                  ),
              },
        ),
        h("output", { "data-testid": "out" }, String(current.value)),
      ]);
  },
});
...
describe("MCarousel", () => {
  ...
  it("switches through v-model:current from the arrows and dots and emits change", async () => {
  ...
  it("disables the edge arrows when loop is off", async () => {
    const onChange = vi.fn();
    const screen = await render(Host, { props: { extra: { loop: false, onChange } } });
  ...
  it("moves with arrow keys on the focused region and jumps with Home / End", async () => {
    const screen = await render(Host, { props: { extra: { loop: false } } });
  ...
  it("uses up / down keys when vertical", async () => {
    const screen = await render(Host, { props: { extra: { direction: "vertical" } } });
    await expect
  ...
  it("autoplays, pauses while hovered and resumes after", async () => {
    const screen = await render(Host, { props: { extra: { autoplay: 120 } } });
  ...
  it("does not render arrows / dots when turned off or with a single slide", async () => {
    const screen = await render(MCarousel, {
      props: { arrows: "none", indicator: "none", items: [{ key: "a", src: "/a.png", alt: "甲" }] },
    });
  ...
  it("takes items as data, writes height and the seeded dot into variables", async () => {
    const screen = await render(Host, {
      props: { asItems: true, initial: 1, extra: { height: 160, seed: 7 } },
    });
    await expect.element(screen.getByText("水的内容")).toBeVisible();
