import type { GoldFleckOptions, PaperPreset } from "../../ink";
...
export interface RicePaperProps {
  ...
  deckleEdge?: boolean;
  /**
   * 底部两侧的远山（左右各一组、一组四层的 SVG 遮罩，照旧站那套手绘远山的结构生成）。默认开：这是旧站宣纸的标志性远景，
   * 同 seed 的图整个会话只生成一次、挂载后才生成，tier 0（省流量 / 减弱动效）下自动不画
   */
  landscape?: boolean;
