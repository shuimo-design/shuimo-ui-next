...
import { MSlider, type MSliderProps, type SliderValue } from ".";

/** 父组件真的把值写回去，连续按键 / 拖动才能基于新值。断言和 Vue 那份逐字一致 */
function Host({ initial, ...rest }: { initial: SliderValue } & Partial<MSliderProps>): ReactNode {
  const [value, setValue] = useState<SliderValue>(initial);
  return (
    <div style={{ width: "400px", padding: "40px" }}>
      <MSlider {...rest} value={value} onValueChange={setValue} />
...
describe("MSlider", () => {
  it("adjusts with the keyboard", async () => {
    ...
    const screen = await render(<Host initial={50} onChange={onChange} />);
  ...
  it("jumps to the clicked position on the rail", async () => {
    const onChange = vi.fn();
    const onInput = vi.fn();
    const screen = await render(<Host initial={0} onChange={onChange} onInput={onInput} />);
  ...
  it("snaps to step and clamps", async () => {
    const screen = await render(<Host initial={0} min={0} max={1} step={0.25} />);
    const thumb = screen.getByRole("slider");
    thumb.element().focus();
  ...
  it("keeps range thumbs from crossing", async () => {
    const screen = await render(<Host initial={[30, 60]} range />);
  ...
  it("does not respond when disabled", async () => {
    const onChange = vi.fn();
    const screen = await render(<Host initial={40} disabled onChange={onChange} />);
    const thumb = screen.getByRole("slider");
  ...
  it("shows min, percent and max when showInfo is on", async () => {
    const screen = await render(<MSlider value={25} min={-50} max={50} showInfo />);
  ...
  it("keeps the bead inside the rail at both ends", async () => {
    const screen = await render(<Host initial={100} />);
  ...
  it("formats the tooltip", async () => {
    const screen = await render(<MSlider value={20} formatTooltip={(v: number) => `${v}%`} />);
    const thumb = screen.getByRole("slider");
