...
import { MSelect, type SelectOption, type SelectValue } from ".";
...
describe("MSelect", () => {
  it("opens on click and picks an option into value", async () => {
    ...
    expect(onValueChange).toHaveBeenCalledWith("shui");
  ...
  it("adds and removes tags in multiple mode", async () => {
    function Host() {
      const [value, setValue] = useState<SelectValue[]>(["shan"]);
      return (
        <div>
          <MSelect
            options={options}
  ...
  it("filters options by typed text", async () => {
    const screen = await render(<MSelect options={options} filterable teleport={false} />);
    await screen.getByRole("combobox").click();
  ...
  it("moves highlight with ArrowDown and picks with Enter", async () => {
    const onValueChange = vi.fn();
    const screen = await render(
      <MSelect options={options} teleport={false} onValueChange={onValueChange} />,
    );
    await screen.getByRole("combobox").click();
    ...
    expect(onValueChange).toHaveBeenCalledWith("yue");
  });

  it("closes with Escape", async () => {
    const onVisibleChange = vi.fn();
    const screen = await render(
      <MSelect options={options} teleport={false} onVisibleChange={onVisibleChange} />,
    );
    await screen.getByRole("combobox").click();
    await expect.element(screen.getByRole("listbox")).toBeVisible();
  ...
  it("does not open when disabled", async () => {
    const onVisibleChange = vi.fn();
    const screen = await render(
      <MSelect options={options} disabled teleport={false} onVisibleChange={onVisibleChange} />,
    );
...
describe("MSelect · 旧版数据格式", () => {
  it("accepts primitive options and shows them as-is", async () => {
    ...
    expect(onValueChange).toHaveBeenCalledWith("丑");
  ...
  it("reads label / value / input text through optionParam, valueParam and inputParam", async () => {
    ...
    expect(onValueChange).toHaveBeenCalledWith("壹");
  });
  ...
  it("passes the raw option and label to renderOption", async () => {
    const screen = await render(
      <MSelect
        options={[{ before: "乾", after: "坎" }]}
        optionParam="before"
        teleport={false}
        renderOption={({ option, label }) => `${label}→${(option as { after: string }).after}`}
      />,
    );
    await screen.getByRole("combobox").click();
  ...
  it("reports typing and calls fetch when the list hits the bottom", async () => {
    const onInput = vi.fn();
