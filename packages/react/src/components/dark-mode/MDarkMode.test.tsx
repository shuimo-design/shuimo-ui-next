...
// 断言和 Vue 那份逐字一致；v-model 在这边是 dark + onDarkChange
describe("MDarkMode", () => {
  beforeEach(() => {
    localStorage.clear();
  ...
  it("renders a switch that reflects the current theme", async () => {
    const screen = await render(<MDarkMode autoMode={false} transition={false} />);
  ...
  it("is disabled", async () => {
    const onChange = vi.fn();
    const screen = await render(
      <MDarkMode disabled autoMode={false} transition={false} onChange={onChange} />,
    );
    const sw = screen.getByRole("switch");
  ...
  it("restores the stored choice on mount", async () => {
    ...
    const screen = await render(<MDarkMode storageKey="my-theme" transition={false} />);
    await expect.element(screen.getByRole("switch")).toHaveAttribute("aria-checked", "true");
    expect(document.documentElement.dataset.theme).toBe("dark");
  });

  it("leaves the theme alone by default", async () => {
    await render(<MDarkMode transition={false} />);
    // 默认亮色纸：没记录也不写 data-theme，更不跟随系统
    await new Promise((r) => setTimeout(r, 50));
    expect(document.documentElement.dataset.theme).toBeUndefined();
  });

  it("follows the system only with autoMode", async () => {
    await render(<MDarkMode transition={false} autoMode />);
    await expect.poll(() => document.documentElement.dataset.theme).toBe("system");
  });

  it("follows the dark prop", async () => {
    const screen = await render(<MDarkMode dark transition={false} />);
    const sw = screen.getByRole("switch");
    await expect.element(sw).toHaveAttribute("aria-checked", "true");
    expect(document.documentElement.dataset.theme).toBe("dark");
    await screen.rerender(<MDarkMode dark={false} transition={false} />);
    await expect.element(sw).toHaveAttribute("aria-checked", "false");
    expect(document.documentElement.dataset.theme).toBe("light");
  });
});
