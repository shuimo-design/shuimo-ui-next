...
import { MDescriptions, MDescriptionsItem, type DescriptionsItem } from ".";
...
function Demo() {
  ...
  const [items, setItems] = useState<DescriptionsItem[]>([...painting]);
  return (
    <div>
      <MDescriptions items={items} bordered={bordered} title="作品" />
      <button onClick={() => setBordered((v) => !v)}>格线</button>
      <button onClick={() => setItems((prev) => [...prev, { label: "藏地", value: "台北" }])}>
        追加
      </button>
    </div>
  );
...
describe("MDescriptions", () => {
  it("renders dt / dd pairs on a grid with the computed positions", async () => {
    const screen = await render(<MDescriptions items={painting} title="作品" />);
  ...
  it("stacks label over value in the vertical layout", async () => {
    const screen = await render(
      <MDescriptions items={painting} layout="vertical" column={2} size="sm" />,
    );
    const root = screen.container.querySelector<HTMLElement>(".m-descriptions")!;
  ...
  it("collects MDescriptionsItem children in written order with their nodes", async () => {
    const screen = await render(
      <MDescriptions column={2} colon={false}>
        <MDescriptionsItem label="形制" value="立轴" />
        <MDescriptionsItem label="状态">
          <b>已修复</b>
        </MDescriptionsItem>
        <MDescriptionsItem key="note" label="说明" span={2} labelNode={<em>备注</em>}>
          画心完好
        </MDescriptionsItem>
      </MDescriptions>,
    );
  ...
  it("renders renderLabel / renderValue and the title / extra nodes", async () => {
    const screen = await render(
      <MDescriptions
        items={painting.slice(0, 2)}
        bordered
        title={<i>标题插槽</i>}
        extra={<button>编辑</button>}
        renderLabel={({ item }) => `「${item.label}」`}
        renderValue={({ item }) => <strong>{String(item.value)}</strong>}
      />,
    );
    const root = screen.container.querySelector<HTMLElement>(".m-descriptions")!;
