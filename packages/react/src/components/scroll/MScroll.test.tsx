...
import { MScroll, type ScrollPosition } from ".";
...
function Host({
  ...
  return (
    <div>
      <MScroll height={200} always={always} style={{ width: "400px" }} onScroll={onScroll}>
        <div data-testid="inside" style={{ width: size.w, height: size.h }} />
      </MScroll>
      <button data-testid="shrink" onClick={() => setSize({ w: 100, h: 100 })} />
    </div>
  );
}
...
describe("MScroll", () => {
  ...
  it("moves the thumb and emits scroll when the view scrolls", async () => {
    ...
    const screen = await render(<Host onScroll={onScroll} />);
  ...
  it("keeps bars visible with always", async () => {
    const screen = await render(<Host always />);
