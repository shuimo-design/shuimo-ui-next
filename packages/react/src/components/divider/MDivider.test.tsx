...
describe("MDivider", () => {
  it("renders a horizontal separator with a brush line sized to its width", async () => {
    const screen = await render(<MDivider thickness={6} />);
  ...
  it("renders vertical", async () => {
    const screen = await render(<MDivider vertical />);
    const root = screen.getByRole("separator");
  ...
  it("renders text between two lines and honours align", async () => {
    const screen = await render(<MDivider text="山水" align="left" />);
  ...
  it("uses the children instead of text", async () => {
    const screen = await render(<MDivider>题跋</MDivider>);
