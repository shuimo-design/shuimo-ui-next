...
describe("MTag", () => {
  ...
  it("wires the three-piece frame and the cross as mask variables", async () => {
    const screen = await render(<MTag closable>底图</MTag>);
  ...
  it("uses the color prop as the ink color", async () => {
    const screen = await render(<MTag color="#951c48">菜头紫</MTag>);
  ...
  it("emits close from the close button without bubbling a click", async () => {
    ...
    const onClick = vi.fn();
    const screen = await render(
      <MTag closable onClose={onClose} onClick={onClick}>
        可关
      </MTag>,
    );
  ...
  it("does not emit close when disabled", async () => {
    const onClose = vi.fn();
    const screen = await render(
      <MTag closable disabled onClose={onClose}>
        禁用
      </MTag>,
    );
