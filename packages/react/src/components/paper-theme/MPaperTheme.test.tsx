...
// 断言和 Vue 那份逐字一致；v-model:preset 在这边是 preset + onPresetChange
describe("MPaperTheme", () => {
  ...
  it("applies the preset to html, remembers it and emits", async () => {
    ...
    const screen = await render(
      <MPaperTheme onPresetChange={onPresetChange} onChange={onChange} />,
    );
    ...
    expect(onPresetChange).toHaveBeenLastCalledWith("antique");
  ...
  it("is disabled", async () => {
    const onChange = vi.fn();
    const screen = await render(<MPaperTheme disabled onChange={onChange} />);
  ...
  it("moves and selects with the arrow keys, Home and End", async () => {
    const onChange = vi.fn();
    const screen = await render(<MPaperTheme onChange={onChange} />);
    const raw = screen.getByRole("radio", { name: "生宣" });
  ...
  it("does not touch storage with storage=false", async () => {
    const screen = await render(<MPaperTheme storage={false} />);
  ...
  it("shows only the given presets with custom labels", async () => {
    const screen = await render(
      <MPaperTheme presets={["raw", "antique"]} labels={{ antique: "绢本" }} />,
    );
    expect(
      screen
        .getByRole("radio")
  ...
  it("follows the preset prop", async () => {
    const screen = await render(<MPaperTheme preset="teaStained" />);
    await expect
      .element(screen.getByRole("radio", { name: "茶染" }))
      .toHaveAttribute("aria-checked", "true");
    expect(html.dataset.paper).toBe("teaStained");
    await screen.rerender(<MPaperTheme preset="raw" />);
    await expect
