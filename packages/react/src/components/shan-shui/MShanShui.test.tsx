...
import { MShanShui, type MShanShuiProps } from ".";
...
function Host({ bannerProps }: { bannerProps?: Partial<MShanShuiProps> }) {
  return (
    <div id="box" style={{ height: 300, overflow: "auto" }}>
      <div style={{ height: 400 }} />
      <MShanShui seed={5} tier={1} height={300} {...bannerProps} />
...
describe("MShanShui", () => {
  it("paints a seeded scene, fades in once the masks decode and emits ready", async () => {
    ...
    const screen = await render(
      <MShanShui seed={11} tier={1} parallax="none" height={320} onReady={onReady}>
        山水
      </MShanShui>,
    );
  ...
  it("tier 0 is flat bands without masks or parallax", async () => {
    const onReady = vi.fn();
    const screen = await render(<MShanShui seed={2} tier={0} height={200} onReady={onReady} />);
    await vi.waitFor(() => expect(onReady).toHaveBeenCalledTimes(1));
  ...
  it("clamps the layer count and drops the optional pieces", async () => {
    ...
    await screen.rerender(
      <MShanShui seed={3} tier={1} layers={1} sun geese={false} boat={false} height={200} />,
    );
    expect(kinds(screen.container)).toEqual([
      "m-shan-shui__sun",
      "m-shan-shui__ridge",
  ...
  it("palette and parallax mode go into the class list, height into the variable", async () => {
    const screen = await render(
      <MShanShui seed={3} tier={1} palette="dusk" parallax="pointer" height="40vh" />,
    );
    const rootEl = screen.container.querySelector(".m-shan-shui") as HTMLElement;
  ...
  it("pointer parallax moves near layers more than far ones and clears when switched off", async () => {
    const screen = await render(<MShanShui seed={5} tier={1} parallax="pointer" height={300} />);
    ...
    await screen.rerender(<MShanShui seed={5} tier={1} parallax="none" height={300} />);
