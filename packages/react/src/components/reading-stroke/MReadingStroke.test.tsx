...
import { MReadingStroke, type MReadingStrokeProps } from ".";
...
function Host({
  ...
  strokeProps?: Partial<MReadingStrokeProps>;
  ...
  return (
    <div>
      ...
      <MReadingStroke target="#box" {...strokeProps} onChange={onChange} />
    </div>
  );
}
...
describe("MReadingStroke", () => {
  ...
  it("emits change only when a whole percent point is crossed", async () => {
    const onChange = vi.fn();
    await render(<Host onChange={onChange} />);
  ...
  it("generates the brush mask from the measured width and applies it under m-ink-ready", async () => {
    ...
    await render(<Host strokeProps={{ seed: 3 }} />);
    const el = bar();
  ...
  it("puts position, thickness, color and z-index into the classes and variables", async () => {
    await render(
      <Host strokeProps={{ position: "bottom", thickness: 6, color: "#861717", zIndex: 20 }} />,
    );
    const el = bar();
  ...
  it("accepts an element or a function as the target", async () => {
    const screen = await render(
      <Host strokeProps={{ target: () => document.querySelector<HTMLElement>("#box") }} />,
    );
    await scrollBoxTo(900);
    await vi.waitFor(() => expect(bar().getAttribute("aria-valuenow")).toBe("50"));
    await screen.rerender(<Host strokeProps={{ target: box() }} />);
