...
import { MTabPane, MTabs, type MTabsProps, type ReactTabPane, type TabName } from ".";
...
function Host(props: {
  initial?: TabName;
  names?: string[];
  extra?: Partial<MTabsProps>;
  /** 用数据数组而不是子组件写法 */
  asPanes?: boolean;
}) {
  const [active, setActive] = useState<TabName | undefined>(props.initial);
  const names = props.names ?? ["shan", "shui", "yun"];
  const panes: ReactTabPane[] = names.map((name) => ({
    name,
    label: LABELS[name],
    disabled: name === "yun",
    content: `${LABELS[name]}的内容`,
  }));
  return (
    <div>
      <MTabs
        ...
        panes={props.asPanes ? panes : undefined}
      >
        {props.asPanes
          ? null
          : names.map((name) => (
              <MTabPane key={name} name={name} label={LABELS[name]} disabled={name === "yun"}>
                {`${LABELS[name]}的内容`}
              </MTabPane>
            ))}
      </MTabs>
      <output data-testid="out">{String(active ?? "")}</output>
    </div>
  );
}
...
describe("MTabs", () => {
  it("renders one tab per pane and shows only the active panel", async () => {
    const screen = await render(<Host initial="shan" />);
  ...
  it("activates the first pane when value is not set", async () => {
  ...
  it("switches through value on click and calls onChange / onTabClick", async () => {
    const onChange = vi.fn();
    const onTabClick = vi.fn();
    const screen = await render(<Host initial="shan" extra={{ onChange, onTabClick }} />);
    ...
    expect(onTabClick).toHaveBeenCalledWith("shui", expect.anything());
  ...
  it("does not respond on a disabled pane", async () => {
    const onChange = vi.fn();
    const screen = await render(<Host initial="shan" extra={{ onChange }} />);
  ...
  it("does not respond when the whole group is disabled", async () => {
    const onChange = vi.fn();
    const screen = await render(<Host initial="shan" extra={{ disabled: true, onChange }} />);
  ...
  it("uses up / down keys when the nav is vertical", async () => {
    const screen = await render(<Host initial="shan" extra={{ position: "left" }} />);
  ...
  it("renders a lazy pane only after it is first activated", async () => {
    const screen = await render(
      <MTabs defaultValue="a">
        <MTabPane name="a" label="甲">
          壹
        </MTabPane>
        <MTabPane name="b" label="乙" lazy>
          贰
        </MTabPane>
      </MTabs>,
    );
  ...
  it("calls onTabRemove from the close button without switching", async () => {
    const onTabRemove = vi.fn();
    const onChange = vi.fn();
    const screen = await render(
      <Host initial="shan" extra={{ closable: true, onTabRemove, onChange }} />,
    );
  ...
  it("falls back to a neighbor when the active pane is removed", async () => {
    function Removable() {
      const [active, setActive] = useState<TabName | undefined>("shui");
      const [names, setNames] = useState(["shan", "shui", "yun"]);
      return (
        <div>
          <MTabs
            value={active}
            onValueChange={setActive}
            panes={names.map((name) => ({ name, label: name, content: name }))}
          />
          <button type="button" onClick={() => setNames(["shan", "yun"])}>
            删掉水
          </button>
          <output data-testid="out">{String(active ?? "")}</output>
        </div>
  ...
  it("renders labelNode and keeps nav order as written", async () => {
    const screen = await render(
      <MTabs defaultValue="a">
        <MTabPane name="a" labelNode={<em>斜体甲</em>}>
          壹
        </MTabPane>
  ...
  it("switches to the card skin without the brush lines", async () => {
    const screen = await render(<Host initial="shan" extra={{ type: "card" }} />);
  ...
  it("renders the extra node at the end of the nav", async () => {
    const screen = await render(
      <MTabs
        extra={<button type="button">新增</button>}
        panes={[{ name: "a", label: "甲", content: "壹" }]}
      />,
    );
