...
import { createGlyphMeasurer, generateStamp } from "../../ink";
import { MStamp } from ".";
...
describe("MStamp", () => {
  it("renders one <text> per character with the seal as an image", async () => {
    const screen = await render(<MStamp text={["水墨", "丹青"]} seed={3} />);
  ...
  it("wires the runtime filters into defs and onto the layers", async () => {
    const screen = await render(<MStamp text="听雨" seed={3} />);
  ...
  it("yin mode masks the body with the text and grid lines", async () => {
    const screen = await render(<MStamp text={["落梅听", "风雪"]} mode="yin" gridLines seed={9} />);
  ...
  it("keeps the same border path for the same seed and changes it with another", async () => {
    // 每次 render 都查自己那个容器：vitest-browser-react 一个测试里连着 render + unmount
    // 会把 React root 用废，所以这里不 unmount，靠 container 把几枚章分开
    const path = async (seed: number) => {
      const screen = await render(<MStamp text="印" seed={seed} shape="square" />);
      return screen.container.querySelector(".m-stamp__border")!.getAttribute("d");
    };
  ...
  it("applies color, font and rotate as component variables", async () => {
    const screen = await render(
      <MStamp text="闲章" color="#1a2847" font="'Songti SC', serif" rotate={-6} />,
    );
    const stamp = screen.getByRole("img").element() as HTMLElement;
  ...
  it("measures with the font the svg actually renders in", async () => {
    ...
    const widthWith = async (font: string) => {
      const screen = await render(
        <MStamp text="水墨" shape="auto" carving={0} bleed={0} font={font} />,
      );
      const svg = svgOf(screen.container.querySelector<HTMLElement>(".m-stamp")!);
    ...
    // 不能比 serif 和 monospace 排出来的宽度不相等：那要求机器上装了两套以上中文字体，
    // Linux CI 通常只有一套，两个通用族落到同一个字面、宽度必然相等，断言就假失败
  ...
  it("re-lays out with measured glyph boxes once the font is ready", async () => {
    const screen = await render(<MStamp text="水墨" shape="auto" carving={0} bleed={0} />);
    const svg = svgOf(screen.getByRole("img").element() as HTMLElement);
