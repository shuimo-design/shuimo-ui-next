...
describe("MAvatar", () => {
  it("renders the image when src loads", async () => {
    const screen = await render(
      <MAvatar src={PIXEL} alt="墨客" size="lg">
        墨
      </MAvatar>,
    );
  ...
  it("falls back to the slot when the image fails to load", async () => {
    const onError = vi.fn();
    const screen = await render(
      <MAvatar src="data:image/png;base64,broken" alt="墨客" onError={onError}>
        墨
      </MAvatar>,
    );
  ...
  it("shows a user icon when there is neither src nor slot", async () => {
    const screen = await render(<MAvatar alt="匿名" />);
  ...
  it("shows the slot without src and accepts a pixel size", async () => {
    const screen = await render(
      <MAvatar variant="square" size={32}>
        山
      </MAvatar>,
    );
  ...
  it("generates the ink ring or brush square and the ragged mask per variant", async () => {
    const circle = await render(<MAvatar size={40} seed={2} />);
    ...
    const square = await render(<MAvatar size={40} seed={2} variant="square" />);
