...
- **回纹的摆法一开始做错了，当天返工**（用户：「4 个小的装饰不太对呀。你看看弹窗 4 个角放置的位置，以及和边框的交互控制是怎么实现的？」）。关键是回纹那张图里画的「框角」**不在图的角上，而在离图边 12px 处**（`ink/assets/lattice.ts` 的坐标画在 32 格网格上、框角在离外沿 6 格 = 12px）。弹窗用 `inset: -12px` 让整张图探出框外，那个框角点正好落在面板角上，图案于是骑在框线上、和边框线首尾相接。第一版我为了不让图案被视口裁掉，直接把 `inset` 改成 0，框角点就跑进纸里 12px、图案整个浮在纸面上和框线断开，`cornerGap` 又 +12px 让实线停得更早，中间再空一段——所以四个角看着是悬空的。
- 正确做法是**把画框线的那一层往里收，而不是把图案往里挪**：纸色挪到 `__frame`（铺满，贴着屏幕边），`__panel` 靠 `__frame` 的 `padding: 12px` 整体内收 12px 并只负责画笔触框，回纹 `inset: 0` 贴着纸边摆——框角点照样落在面板角上，和弹窗的接口关系完全一致，`cornerGap` 也用回弹窗那套数值。面板内边距各减 12px（外面那 12px 由包裹层出了）。
- `size` prop 现在是整张纸的宽 / 高（`__frame`），面板比它小 24px；测试量的是 `__frame`。
...
- 挂牌横跨框线（一半在纸外）这条保留：它挂的是朝屏幕内那一侧的边，外面就是遮罩，永远不会被视口裁到，和弹窗的观感也一致。竖边牌顶从 42px 下移到 58px——回纹收进来之后往下占到 47px，42px 会贴上去。
- 横向抽屉（上 / 下滑出）的挂牌**都贴右上角**（2026-09-11 用户要求，原来是上滑出挂底边、下滑出挂顶边，位置不一致）。下滑出的上框线是朝屏幕内的自由边，牌照常横跨它；上滑出的上框线顶着视口，横跨会被裁掉半个牌，所以整块收进纸里、只留 14px 让牌尖上那截绳子露出来。两个都离右端 76px，避开占 64px 的右上角回纹。
