...
| M1 特效引擎（2 周）        | `ink/` 八个模块 + RicePaper + Border + Stamp + DarkMode                                                          | playground 首页：程序化宣纸 + 山 + 视差 + 墨晕，tier 0/1/2 可切，首屏 JS < 150KB gzip —— **进行中（2026-09-08）**：paper / landscape / parallax / bleed / MRicePaper 已完成并有浏览器测试；stroke / stamp / reveal / transition / MBorder / MStamp / MDarkMode 未做 |
