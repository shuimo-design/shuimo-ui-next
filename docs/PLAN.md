...
- 横向抽屉（上 / 下滑出）的挂牌**都贴右上角**（2026-09-11 用户要求，原来是上滑出挂底边、下滑出挂顶边，位置不一致）。两个用同一条规则：牌整块收在纸里、牌顶离上框线 14px（牌尖上那截绳子正好跨过框线露出来），离右端 72px 避开右上角回纹（回纹画幅 64px、探出框外 12px，沿框线往里占到 52px）。
