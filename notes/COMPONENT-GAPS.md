...
| 组件               | 说明                                                   | 积木                                |
| ------------------ | ------------------------------------------------------ | ----------------------------------- |
| MShanShui 山水横幅 | hero：远山剪影 + 朱砂日 + 雁阵 + 孤舟，滚动 / 鼠标视差 | `inkMountainScene`、scene、parallax |

做过一版（`34da98d`，已回滚），画面不成立。远山的生成器已经在 MRicePaper 里定稿了（`ink/assets/mountain.ts`，见 PLAN「宣纸远山重做」），横幅可以直接用它铺底，剩下朱砂日 / 雁阵 / 孤舟的样张。
