...
- **三个包都要开 `pack.unbundle: true`（2026-09-13）**：一个源文件出一个产物文件。不开的话，rolldown 会把整个包合成一个 `index.js`，barrel 用运行时的 `__exportAll` 把所有组件挂到一个对象上，core 的两个入口还会共用一个 150 KB 的公共块 —— 使用方的打包器静态分析不动，**只用一个组件也要把整份产物留下**。实测（压缩 + gzip，减掉框架基线）：一个头像从 45.4 KB 降到 6.4 KB，一个按钮从 44.8 降到 8.2，React 侧同理从 45.9 降到 5.9；用全量组件的场景几乎不变（Vue 持平，React +2 KB）。副作用：core 的 dist 从 3 个文件变成 309 个，两个壳各 200 多个。另外 `unbundle` 打开后 CSS 默认跟着拆（会变成 `dist/styles/index.css`），要显式 `css: { splitting: false }` 才还是一份 `dist/style.css`。
...
- playground 已改为「左侧组件菜单 + 右侧单组件示例」布局（hash 路由，一个组件一个示例）。**2026-09-13 起拆成两版**：`/vue/` 和 `/react/` 各是一个独立的单框架应用（各自的 `index.html` 和外壳），组件清单、API 数据整形、外壳样式放在 `playground/src/shared/`，示例分别在 `src/vue/demos/*.vue` 和 `src/react/demos/*.tsx`。新组件要在 `src/shared/catalog.ts` 里加一条，并且两边各补一个示例（缺哪边启动就报错，不会静默少一页）；没有独立页面的子组件（MTableColumn、MFormItem 这类）在清单里挂到主组件的 `parts` 上，否则文档生成了没人看得见。每页示例下面有源码区：`?highlight` 这个自定义查询在**构建期**用 shiki 把示例文件高亮好（浏览器里不带高亮器），点开才按需加载那一份 —— 47 份一起 eager 打进主包会让它从 306 KB 涨到 2 MB。
...
**装饰全部收进纸里面**（2026-09-11 当天返工，用户原话「能不能把这些装饰以合理的方式移到抽屉里面」）：抽屉贴着屏幕边、满高（或满宽），装饰只要探出纸外就会被视口裁掉。第一版是反过来做的——让纸离屏幕边留 16px，好给探出去的装饰腾地方，但那样抽屉就不贴边了。改成让纸贴回屏幕边，装饰往里收：

- **四角回纹绕了两圈弯路，最后回到第一版**。关键是回纹那张图里画的「框角」**不在图的角上，而在离图边 12px 处**（`ink/assets/lattice.ts` 的坐标画在 32 格网格、框角在离外沿 6 格 = 12px）。共用样式用 `inset: -12px` 让整张图探出框外，那个框角点才正好落在面板角上，图案骑在框线上、和边框线首尾相接；`cornerGap` 让实线在角饰第一根条处停笔，接口就看不出缝。**所以纸必须离屏幕边留出至少 12px**，探出去那部分才不会被视口裁掉——第一版给根节点留 16px 就是为这个。
- 中间试过两种「让纸贴边」的改法，都不对，已回退：① 直接把回纹改成 `inset: 0` 并把 `cornerGap` +12px——框角点跑进纸里 12px，图案整个浮在纸面上、和框线断开，中间还空一段；② 纸色挪到 `__frame` 铺满、`__panel` 靠 `padding: 12px` 内收——接口关系是对的，但纸和墨框之间多出一圈 12px 的白边，不是原来的样子。用户原话：「最开始你添加进来的角和边的结合是对的，只是挂饰出了问题。」
- 弹窗为了让回纹探出去那部分露在纸外，把四角挖空了；抽屉的回纹最远只到纸边，没有露到纸外的部分，不需要挖，纸保持完整长方形。
- 题头小景（远山 / 枯树 / 朱砂日）**抽屉上先不要了**（2026-09-11 用户原话「把抽屉里面山和树的那个装饰品可以先隐藏掉，我看看」）。不是用 CSS 藏起来，是连生成一起去掉，免得白算一张 SVG；要恢复就从这条提交里把 `inkSceneSvg` 那几行捡回来。面板上内边距仍保留 66px。
- 挂牌横跨框线（一半在纸外）这条保留：它挂的是朝屏幕内那一侧的边，外面就是遮罩，永远不会被视口裁到，和弹窗的观感也一致。竖边牌顶从弹窗的 42px 下移到 58px——弹窗挂的是右边、右上角那套回纹小，抽屉左上角这套往下挂到 35px，42px 会让牌顶的墨花蹭上去。
- 横向抽屉（上 / 下滑出）的挂牌**挂右边竖框线、贴着右上角**（2026-09-11 用户要求，先是「都贴右上角」、又补了一句「靠的是右边框右上角」）。中间试过「牌整块收在纸里、离上框线 14px、离右端 72px」，那是挂在上框线内侧、牌没跨线，不是用户要的；改成和竖向抽屉同一套做法：牌横跨右边竖框线、探出框外一半 11px，牌顶离上框线 42px（和弹窗同一个数——右上角那套回纹只往下挂到 29.5px，不会打架）。四个方向于是统一成一句话：牌永远横跨一条竖框线，左滑出挂右边线、右滑出挂左边线、上下滑出挂右边线。
...
用户原话「目前我们宣纸的远山的效果勉勉强强能用……最好是贴近原生的，上一版的 shuimo-ui 的样子」。旧站那八张手绘 webp 是人画的，我们在用尺子和噪声模仿手，所以整个过程是**先量旧图、再照数字做、再并排比**（`bench/scene-sample.browser.ts` 出一张对比样张：旧站整幅按旧 CSS 复原 / 新版两个 seed / 页面透明度下的三方对比 / 左右组放大）。做完的东西在 `ink/assets/mountain.ts`，接在 `MRicePaper` 里。

### 旧图量出来的结构（照这个做的）

- 一侧一组四层，从后往前 base（最高最宽，青绿渲染）→ mid（矮六成、靠外侧）→ front（外侧角上一座小山，深一档、半透明）→ front2（往里偏一点的中等尖峰）。**最高的山在最后面，越往前越矮、越深、越清楚**——第一版把近山做成又大又实的那座，前后关系整个反了。
- 线约 4096 宽画幅的 0.3%、近黑；青绿颜料 ≈ rgb(60 85 78)，前景两座 ≈ (59 69 66)；山体 alpha 从脊线下 0.8 → 20% 高处 0.55 → 50% 处 0.26；前景淡得更快。整组落到页面上是 0.3 透明度。
- 山是**连绵的钝三角**：峰按位置排成一串，相邻两峰之间落到鞍部就又抬起来，坡度 45° 上下、顶角 100° 以上；坡线上是一丛丛的小褶皱（主峰附近密、坡下段稀），不是直刀锋也不是圆馒头。
- 山体里的空白**不是留白，是云雾把山连线一起盖住了**；旧图每一组底下四分之一几乎是空的。

### 做法

- 几何：主链（峰 + 鞍部）→ 每段坡叠三层噪声（3.5% 高的脊状褶皱、1% 起伏、0.5% 锯齿；褶皱密度按高度走）。相邻峰的高差不许超过横向距离的 0.3 倍，坡度封顶 0.85——两条规矩缺一个，主峰旁边就会陡成针（"摔碎的玻璃"）。「出峰顶先走一段缓坡」那种峰肩试过，出来是馒头和塔，钝三角的正确做法是整条坡都不许陡。
- 线：**真笔触**。从旧 webp 里抠墨线（亮度 < 34 且不透明的像素连成块，长线按 50–160px 切段，每段换成「沿线位置 + 离中线偏移」的归一化多边形，锯齿多的一侧统一朝天），得到 87 条，存成 `mountain-brushes.ts`（41 KB 源码，矢量，不是位图）；生成时沿每条露出的棱线按弧长一条接一条弯上去。皴用短笔库。
- 山体、雾、剪影仍是程序的：噪声斑驳 + 亮斑 + 竖向水痕；云雾是几团扁椭圆用低频噪声啃成云形再化开。**山体也试过真纹理**（把旧图山体按脊线展平、浓度分 10 档描成多边形、再按自己的脊线弯回去），alpha 曲线能对上、水痕亮斑都是原样的，但用户看了说还原成程序渲染——那条路的代码没进 src，抠图脚本在会话 scratchpad 的 `extract-wash.py`。
- 每层四张 alpha 遮罩由 CSS 上色：纸色剪影垫底（挡住后面那层）→ 山体（`--m-rice-paper-landscape-wash`；前景 `-front`）→ 墨线（`-color`）→ 纸色云雾盖顶。三种颜色都掺 `--m-ink`，暗色主题自动变灰绿。整组透明度 0.45（同样 0.3 时生成的山比旧图浅一成半，落到带纹理和洒金的纸上更发白）。
- 体积：八层图约 550 KB（四张图各自只带自己的滤镜、羽化的带子每 3 点取 1、笔触多边形 approxPolyDP 1.2），从 1.38 MB 压下来的；再压要牺牲效果。所以**远山的图只在挂载后生成**（快照多了 `mounted`），不进服务端 HTML——它们本来要等 `ready` 才淡入。生成一组 20–40 ms，同 seed 缓存 16 份。

### 踩过的坑

- **前后层反了**：代码里 depth=1 叫近山但放得又矮又小、还整个泡在雾里，前面那座被后面那座切断。看画面判断谁是前山，别看变量名。
- **`<mask>` / `<filter>` 的默认作用范围是被套元素的包围盒外扩 10%**：云雾那一层的包围盒只有几团椭圆大，化开的边一超出去就被切成方框（"飞机尾气"）。四个遮罩和雾的滤镜全改成 `userSpaceOnUse` 按整张画幅算。样张画在 canvas 上没显出来，页面上做 CSS 遮罩才露的——**样张定稿后还是要在真组件里看一眼**。
- 遮罩里画第二个矩形是盖住不是相乘；两个渐变要各套一层 `<g mask>`。内侧淡出的渐变要用画幅坐标锚在边上，按包围盒算的话 0 那一档落在画幅外面、画幅边上还剩两成不透明，留一条细缝。
- 前山发灰不是曲线的问题：一是墨色层的斑驳噪声把 alpha 啃掉将近一半，二是整组底部的雾从 50% 高就开始吃、前山脊线本来就在 55% 高。先量再改。
- 页面 0.3 透明度下细节全看不见，**每一轮都要同时看不透明放大图和页面透明度图**，两张图的问题不一样（放大图看笔触，透明度图看整体深浅和轮廓）。
